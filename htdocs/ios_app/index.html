...
[APP BUSINESS LOGIC]
--->
<script src="https://destruct.co:8080/socket.io/socket.io.js"></script>
<script>
				var
					socket 			= io('https://destruct.co:8080', {secure: true, 'transports': ['websocket', 'xhr-polling']})
					, is_online 	= 0
					, DEBUG_MODE 	= false
				;
			...
			$(function(){
				...
				socket.on('verify-true', function(res) {
					...
					socket.emit("send-user-verify", {me: id_obj.username, user: name});
					$(".add-contact-txt").val("");
					openPage("#app_page", "left", "left");
				});
				
				socket.on('verify-false', function(res) {
					$(".add-contact-txt").css({border: "1px solid red"}).val("");
				...
				var openPage = function(id, new_up_down, old_up_down, cb) {
					...
					if (old) {
						switch (old_up_down) {
							...
							break;
                            case "left":
                                old.animate({top:0, left: -parseInt($(window).width(), 10)}, {complete: function(){
                                    $(this).addClass("hiding").removeClass("page-open");
                                }});
                            break;
                            case "right":
                                old.animate({top:0, left: parseInt($(window).width() * 2, 10)}, {complete: function(){
                                    $(this).addClass("hiding").removeClass("page-open");
                                }});
                            break;
						}
					...
					switch (new_up_down) {
						...
						break;
                          case "left":
                              page
                              .css({top:0, left: parseInt($(window).width() * 2, 10)})
                              .removeClass("hiding")
                              .animate({top: 0, left: 0}, {complete: function(){
									$(this).addClass("page-open");
									 if ("function" === typeof cb) {
										 cb();
									 }
                          ...
                          case "right":
                            page
                            .css({top:0, left: -parseInt($(window).width(), 10)})
                            .removeClass("hiding")
                            .animate({top: 0, left: 0}, {complete: function(){
								$(this).addClass("page-open");
								if ("function" === typeof cb) {
			  ...
			  var addMessage = function(n, m, f) {
				...
				var myId = active_id_object();
				 ...
				 if ("string" == typeof f && n !== myId.username) {
					if (!messages[n]) {
					   messages[n] = {};
					   messages[n]["info"] = {"created": new Date().getTime()}
				...
				setTimeout(function(){
					$("#app_intro_load").fadeOut({duration:1000, queue:false});
                ...
                $(".app-message-list").on("click", ".app-open-conversation", function(){
                	...
                	openPage("#app_conversation_view", "left", "left", function(){
						...
						var active_obj = {user:user, key:window.localStorage.getItem(user+"_key")};
